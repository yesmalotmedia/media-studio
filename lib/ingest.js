...
const DEFAULTS = {
  ...
  minDate: null,
  // CONFIRMED FOR REAL (2026-09-12): the local disk filled up completely
  // (ENOSPC) — real sessions run up to ~23GB PER CAMERA, 3 cameras every
  // ~6-7 hours, and local copies only get cleaned up once every segment from
  // them is reviewed+exported (lib/store.js), so a review backlog directly
  // becomes a disk-space countdown. The crash didn't just fail one copy — it
  // corrupted data/ingest.json (see ingestStore.js's atomic-write fix) and
  // took the whole server down. Refuse to START a new copy at all once free
  // space drops below this floor — better to fall behind on ingestion (fully
  // recoverable: the source stays on \\Servermedia) than to crash and risk
  // store corruption again.
  minFreeSpaceGB: 40,
};

// Free space on the drive holding `p`, in GB — null if it can't be
// determined (older Node without fs.statfs, or a transient stat failure);
// callers should treat null as "unknown, don't block on it" rather than 0.
function freeSpaceGB(p) {
  try {
...
export function startIngestWatcher() {
  ...
  // CONFIRMED FOR REAL (2026-09-13): setInterval fires every pollIntervalSec
  // NO MATTER WHAT, even if the previous tick's poll() is still deep inside
  // an `await ingestOne()` copying a 20GB file (which routinely takes far
  // longer than 60s). That let a SEPARATE, fully independent poll() run
  // concurrently, discover other files as newly-stable in its own pass, and
  // start copying THEM too — with 3 cameras finishing in near-lockstep, this
  // is exactly how dozens of multi-GB copies ended up running at once and
  // filled the disk in under two hours despite the minFreeSpaceGB guard (that
  // guard checks free space once per file right before it starts, which is
  // correct in isolation but meaningless against N other copies started the
  // same way moments earlier that haven't finished consuming their share
  // yet — a check-then-act race, not a real reservation). The real fix is
  // upstream of the guard: never let two poll() calls run at the same time
  // in the first place, so at most one copy is ever in flight and the
  // space check's assumption (nothing else is concurrently consuming space)
  // is actually true when it runs.
  let inFlight = false;
  const tick = async () => {
    if (inFlight) { console.log('[ingest] previous poll still running, skipping this tick'); return; }
    inFlight = true;
    try { await poll(); } catch (e) { console.error('[ingest] poll error', e); } finally { inFlight = false; }
  };
  tick();
...
export async function poll() {
  ...
  for (const entry of entries) {
    ...
    if (stableCount >= cfg.stableChecksRequired) {
      const free = freeSpaceGB(localFolder);
      if (free != null && free < cfg.minFreeSpaceGB) {
        console.warn(`[ingest] only ${free.toFixed(1)}GB free (< ${cfg.minFreeSpaceGB}GB floor) — skipping copy of`, entry.name, 'until space frees up (review+export more approved segments to trigger cleanup)');
        continue; // stays 'watching' with stableCount intact — picked up automatically once space frees up, no data lost
      }
